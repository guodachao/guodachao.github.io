...
![favorites_chrome]({{ site.url }}/assets/images/2016/favorites_chrome.jpg)  
...
主页  
![favorites_chrome]({{ site.url }}/assets/images/2016/favorites_index.png)  

注册  
![favorites_chrome]({{ site.url }}/assets/images/2016/favorites_register.png)  

首页  
![favorites_chrome]({{ site.url }}/assets/images/2016/favorites_home.png)  

收藏  
![favorites_chrome]({{ site.url }}/assets/images/2016/favorites_collect.png)  


## 技术点

这段时间我们团队主要在学习spring boot,这个开源项目也就成了我们的练习新技术的一个非常好的产品，主要的技术都是和spring boot相关，可以参考我以前文章 [spring boot系列文章]({{ site.url }}/spring-boot.html)
...
前端页面由[Angle - Bootstrap Admin theme](https://wrapbootstrap.com/theme/angle-bootstrap-admin-template-WB04HF123)这套主题改造而来;模版引擎使用了```thymeleaf```，可以参考这篇文章：[springboot(四)：thymeleaf使用详解]({{ site.url }}/springboot/2016/05/01/spring-boot-thymeleaf.html)
 
**持久层**

数据库主要使用了 Spring data jpa模版来实现，可以参考这篇文章：[springboot(五)：spring data jpa的使用]({{ site.url }}/springboot/2016/08/20/spring-boo-jpa.html)

**session**

session使用持久化技术来保存登录状态，登录一次保持需要会话30天，主要是依赖redis来实现，参考：[springboot(三)：Spring boot中Redis的使用]({{ site.url }}/springboot/2016/03/06/spring-boot-redis.html)
